...
export default function GoldInteractiveSection() {
  ...
  const loadDataAndHistory = async () => {
    ...
    try {
      ...
      // 1. Definisi key yang dipantau (Sudah disesuaikan dengan filter API Route)
      const monitoredKeys = [
        { brand: 'ANTAM', resource: 'antam', searchBrand: 'ANTAM' },
        { brand: 'ANTAM MULIA RETRO', resource: 'galeri24', searchBrand: 'ANTAM MULIA RETRO' },
        { brand: 'UBS', resource: 'galeri24', searchBrand: 'UBS' },
        { brand: 'GALERI 24', resource: 'galeri24', searchBrand: 'GALERI 24' },
        { brand: 'EMASKU', resource: 'hartadinata', searchBrand: 'EMASKU' }
      ];

      // 2. Inisialisasi tracker untuk melacak apakah histori tiap brand sudah ditemukan
      const trackers: Record<string, { current: number; oldPrice: number; found: boolean }> = {};
      
      monitoredKeys.forEach(({ brand, resource, searchBrand }) => {
        const currentData = res.rawMarketData?.find(
          (d: any) => d.brand.toUpperCase() === searchBrand && d.resource.toLowerCase() === resource
        );
        trackers[brand] = {
          current: currentData ? Number(currentData.sell_price) : 0,
          oldPrice: 0,
          found: false
        };
      });

      // 3. Loop mundur maksimal 7 hari untuk melengkapi data yang kosong
      for (let i = 1; i <= 7; i++) {
        // Optimasi: Hentikan fetch API jika SEMUA brand sudah menemukan data historinya
        if (Object.values(trackers).every(t => t.found)) break;

        const backDate = new Date();
        backDate.setDate(backDate.getDate() - i);
        const dateString = backDate.toISOString().split('T')[0];
        
        try {
          const historyRes = await fetch(`${baseUrl}/api/gold-price?date=${dateString}`);
          const tempJson = await historyRes.json();
          
          if (tempJson.data && tempJson.data.length > 0) {
            monitoredKeys.forEach(({ brand, resource, searchBrand }) => {
              // Jika brand ini BELUM punya histori, coba cari di data tanggal ini
              if (!trackers[brand].found) {
                const prevData = tempJson.data.find(
                  (d: any) => d.brand.toUpperCase() === searchBrand && d.resource.toLowerCase() === resource
                );
                
                if (prevData && Number(prevData.sell_price) > 0) {
                  trackers[brand].oldPrice = Number(prevData.sell_price);
                  trackers[brand].found = true;
                }
              }
            });
          }
        } catch (e) {
          console.error(`Gagal fetch history untuk tanggal ${dateString}:`, e);
          // Biarkan loop berlanjut ke hari berikutnya jika fetch gagal
        }
      }

      // 4. Kalkulasi hasil akhir selisih trend
      const histories: any = {};
      monitoredKeys.forEach(({ brand }) => {
        const { current, oldPrice } = trackers[brand];

        if (current > 0 && oldPrice > 0) {
          ...
          // Fallback jika tidak ada data current atau histori gagal didapat sama sekali
          histories[brand] = { 
            change: 0, 
            percent: "0.00", 
            isUp: false, 
  ...
  const getMarketData = (tabName: string) => {
    ...
    if (tabName === "Antam Certicard" || tabName === "Antam Retro") return data.rawMarketData.find((d: any) => d.brand.toUpperCase() === 'ANTAM');
    ...
    <section id="gold-interactive-section" className="py-10 md:py-24 bg-white dark:bg-[#0A0B0D] relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-8 md:mb-16">
          <div className="flex items-center gap-2 px-3 py-1 rounded-full border border-[#0b213b]/10 bg-white dark:bg-[#111318] text-[#0b213b] dark:text-[#C9A961] mb-4 shadow-sm border-t-2 border-t-[#C9A961]">
            <Clock size={12} className="text-[#C9A961]" />
            <span className="text-[10px] font-bold tracking-widest">{currentTime}</span>
          </div>
          
          <h2 className="text-3xl md:text-6xl font-serif text-[#0b213b] dark:text-white mb-6 leading-tight">
          ...
          <div className="flex items-center gap-3 bg-white dark:bg-[#111318] px-4 py-3 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
            ...
            <div className="text-left">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-bold text-[#0b213b] dark:text-white">{formatTrendIDR(headerHistory.change)}</span>
        ...
        <div className="flex justify-center mb-8">
          <div className="grid grid-cols-3 w-full max-w-lg bg-[#0b213b]/5 dark:bg-[#111318] p-1 rounded-2xl border border-gray-100 dark:border-gray-800">
            {tabs.map((tab) => (
              <button
                key={tab} onClick={() => setActiveTab(tab)}
                className={`relative py-3 rounded-xl text-[9px] md:text-xs font-black tracking-tighter transition-all duration-300 ${
                  activeTab === tab ? "text-white" : "text-[#0b213b]/60 dark:text-gray-500"
                }`}
              >
                {activeTab === tab && <motion.div layoutId="tab-active" className="absolute inset-0 bg-[#0b213b] dark:bg-[#C9A961] rounded-xl shadow-lg" />}
        ...
        <AnimatePresence mode="wait">
          {activeTab === "Perhiasan" ? (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="bg-white dark:bg-[#111318] p-8 md:p-20 rounded-3xl text-center border-b-4 border-b-[#C9A961] shadow-xl">
              ...
              <h3 className="text-xl md:text-4xl font-serif text-[#0b213b] dark:text-white mb-4 italic">Buyback Perhiasan</h3>
              ...
              <button onClick={() => handleWhatsApp('jewelry')} className="w-full md:w-auto px-10 py-4 bg-[#0b213b] dark:bg-[#C9A961] text-white dark:text-[#0b213b] rounded-xl font-black text-[10px] uppercase flex items-center justify-center gap-3 mx-auto shadow-xl">KONSULTASI VIA WHATSAPP <ChevronRight size={16} /></button>
            </motion.div>
          ) : (
            <div className="flex flex-col lg:grid lg:grid-cols-12 gap-6">
              <div className="lg:col-span-5 space-y-6">
                <div className="bg-[#0b213b] p-6 rounded-3xl text-white border-r-4 border-r-[#C9A961] shadow-xl relative overflow-hidden group">
                ...
                <div className="bg-white dark:bg-[#111318] p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm">
                  <div className="flex items-center gap-3 mb-6">
                    <Calculator className="text-[#0b213b] dark:text-[#C9A961]" size={18} />
                    <h4 className="font-bold text-[9px] tracking-widest uppercase">Simulasi Transaksi</h4>
                  </div>
                  <div className="space-y-6">
                    <div className="relative group">
                      <input type="number" value={customGram} onChange={(e) => setCustomGram(parseFloat(e.target.value) || 0)} className="w-full p-4 bg-gray-50 dark:bg-[#0A0B0D] rounded-xl text-2xl font-serif font-bold text-[#0b213b] dark:text-white outline-none border border-gray-100 dark:border-gray-800 focus:ring-1 focus:ring-[#C9A961]"/>
                    ...
                    <div className="p-5 bg-[#0b213b] dark:bg-white/5 rounded-2xl border border-[#C9A961]/20">
                      ...
                      <button onClick={() => handleWhatsApp('buy')} className="w-full py-4 bg-[#C9A961] text-[#0b213b] rounded-xl font-black text-[9px] shadow-lg flex items-center justify-center gap-2 uppercase tracking-widest"><ShoppingBag size={14} /> Pesan Sekarang</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-7">
                <div className="bg-white dark:bg-[#111318] rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden h-full">
                  <div className="p-5 border-b border-gray-50 dark:border-gray-800 bg-gray-50/50 dark:bg-white/5 flex items-center gap-2">
                    ...
                    <h4 className="text-[9px] font-black tracking-widest text-[#0b213b] dark:text-white uppercase">Daftar Harga Final</h4>
                  </div>
                    ...
                    {denominations.map((gram) => (
                      <div key={gram} className="p-4 flex justify-between items-center hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-[#0b213b] text-[#C9A961] flex items-center justify-center font-bold text-[9px]">{gram}</div>
                          <span className="font-bold text-[#0b213b] dark:text-white text-[11px]">{gram} Gram</span>
                        </div>
        ...
        <div className="mt-16 md:mt-32 pt-12 border-t border-gray-100 dark:border-gray-800">
          <h3 className="text-2xl md:text-4xl font-serif text-[#0b213b] dark:text-white italic opacity-40 mb-10 text-center">Bursa Harga (1g)</h3>
            ...
            {comparisonBrands.map((b) => {
              ...
              return (
                <div key={b.name} className="group bg-white dark:bg-[#111318] p-8 rounded-[32px] border border-gray-100 dark:border-gray-800 flex flex-col gap-6 hover:shadow-2xl border-b-4 border-b-[#C9A961] relative overflow-hidden transition-all duration-500">
                  <div className="absolute -right-4 -top-4 text-[#0b213b]/5 group-hover:scale-110 transition-transform"><TrendingUp size={100} /></div>
                  <div className="flex justify-between items-start relative z-10">
                    <div>
                      <p className="text-[10px] font-black text-[#C9A961] uppercase mb-1 tracking-widest">Pasar Resmi</p>
                      <h5 className="text-xl font-bold text-[#0b213b] dark:text-white uppercase">{b.name}</h5>
                    </div>
                  ...
                  <div className="relative z-10">
                    <p className="text-3xl md:text-4xl font-serif text-[#0b213b] dark:text-white">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className="bg-[#0b213b] p-8 rounded-[40px] text-center relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6 text-left">
              ...
              <button onClick={() => handleWhatsApp('jewelry')} className="group px-8 py-4 bg-[#C9A961] text-[#0b213b] rounded-2xl font-black text-xs flex items-center gap-3 hover:bg-white transition-all shadow-xl shadow-[#C9A961]/20">TANYA HARGA SEKARANG <ChevronRight size={18} className="group-hover:translate-x-1 transition-transform" /></button>
            </div>
          </motion.div>
        </div>
      </div>
